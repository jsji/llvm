...
#ifdef __SYCL_DEVICE_ONLY__
#ifndef __NVPTX__
    sycl::vec<int, coordSize + 1> coordsWithLayer{coords, arrayLayer};
#endif
  if constexpr (detail::is_recognized_standard_type<DataT>()) {
#if defined(__NVPTX__)
...
#else
    return __invoke__ImageRead<DataT>(
        detail::convert_handle_to_image<OCLImageArrayTyRead<coordSize>>(
            imageHandle.raw_handle),
        coordsWithLayer);
#endif
  } else {
...
void write_image_array(const unsampled_image_handle &imageHandle
  ...
  if constexpr (detail::is_recognized_standard_type<DataT>()) {
...
#else
    __invoke__ImageWrite(
        detail::convert_handle_to_image<OCLImageArrayTyWrite<coordSize>>(
            imageHandle.raw_handle),
        coordsWithLayer, color);
#endif
  } else {
    // Convert DataT to a supported backend write type when user-defined type is
    // passed
    ...
    __invoke__ImageWrite(
        detail::convert_handle_to_image<OCLImageArrayTyWrite<coordSize>>(
            imageHandle.raw_handle),
        coordsWithLayer, detail::convert_color(color));
#endif
  }
#else
  assert(false); // Bindless images not yet implemented on host.
